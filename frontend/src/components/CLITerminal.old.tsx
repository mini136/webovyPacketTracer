      ...
      if (trimmed === 'configure terminal' || trimmed === 'conf t') {
        ...
        device.data.interfaces.forEach(iface => {
          ...
          if (device.data.type === 'switch') {
            if (iface.trunkMode) {
              newHistory.push(` switchport mode trunk`);
              if (iface.allowedVlans && iface.allowedVlans.length > 0) {
                newHistory.push(` switchport trunk allowed vlan ${iface.allowedVlans.join(',')}`);
              }
              if (iface.nativeVlan && iface.nativeVlan !== 1) {
                newHistory.push(` switchport trunk native vlan ${iface.nativeVlan}`);
              }
            } else {
              newHistory.push(` switchport mode access`);
              if (iface.vlanId && iface.vlanId !== 1) {
                newHistory.push(` switchport access vlan ${iface.vlanId}`);
              }
            }
          }
          ...
          if (iface.subInterfaces && iface.subInterfaces.length > 0) {
            iface.subInterfaces.forEach(subIface => {
              newHistory.push(`!`, `interface ${subIface.name}`);
              newHistory.push(` encapsulation dot1Q ${subIface.vlanId}`);
              if (subIface.ipAddress) {
                newHistory.push(` ip address ${subIface.ipAddress} ${subIface.subnetMask || '255.255.255.0'}`);
              }
              if (subIface.description) {
                newHistory.push(` description ${subIface.description}`);
              }
            });
          }
        });
        ...
        // VLANs for switches
        if (device.data.type === 'switch' && device.data.vlans && device.data.vlans.length > 0) {
          device.data.vlans.forEach(vlan => {
            if (vlan.id !== 1) {
              newHistory.push(`!`, `vlan ${vlan.id}`, ` name ${vlan.name}`);
            }
          });
        }
        ...
        device.data.interfaces.forEach(iface => {
          ...
          if (iface.subInterfaces && iface.subInterfaces.length > 0) {
            iface.subInterfaces.forEach(subIface => {
              ...
              newHistory.push(`${subIface.name.padEnd(22)} ${subIp.padEnd(15)} YES manual ${status.padEnd(21)} ${protocol}`);
            });
          }
        });
        newHistory.push('');
      } else if (trimmed === 'show interfaces trunk' || trimmed === 'sh int trunk') {
        if (device.data.type === 'switch') {
          newHistory.push('Port        Mode         Encapsulation  Status        Native vlan');
          device.data.interfaces.filter(iface => iface.trunkMode).forEach(iface => {
            const mode = 'trunk';
            const encap = '802.1q';
            const status = 'trunking';
            const native = iface.nativeVlan || 1;
            newHistory.push(`${iface.name.padEnd(11)} ${mode.padEnd(12)} ${encap.padEnd(14)} ${status.padEnd(13)} ${native}`);
          });
          newHistory.push('');
          newHistory.push('Port        Vlans allowed on trunk');
          device.data.interfaces.filter(iface => iface.trunkMode).forEach(iface => {
            const vlans = iface.allowedVlans?.join(',') || '1';
            newHistory.push(`${iface.name.padEnd(11)} ${vlans}`);
          });
          newHistory.push('');
        } else {
        ...
        const switchCommands = device.data.type === 'switch' ? [
      ...
      if (trimmed === 'exit' || trimmed === 'end') {
        ...
        if (parts.length > 1) {
          ...
          if (ifaceName.includes('.')) {
            ...
            const parentIface = device.data.interfaces.find(i => 
              i.name.toLowerCase() === parentName.toLowerCase()
            );
            
            if (parentIface) {
              // Sub-interface is valid - enter interface config mode
              setCurrentInterface(ifaceName);
              setMode('interface');
              newHistory.push('');
            } else {
              newHistory.push(`% Invalid parent interface: ${parentName}`);
            }
          } else {
            // Regular interface
    ...
    // Interface configuration mode
      ...
      if (trimmed === 'exit') {
        ...
        if (parts.length >= 4) {
          ...
          if (currentInterface.includes('.')) {
            const [parentName] = currentInterface.split('.');
            const interfaces = device.data.interfaces.map(iface => {
              if (iface.name === parentName) {
                ...
                const subIdx = subInterfaces.findIndex(si => si.name === currentInterface);
                ...
                if (subIdx >= 0) {
                  ...
                  // Create new sub-interface if it doesn't exist
                  subInterfaces.push({
                    name: currentInterface,
                    vlanId: parseInt(currentInterface.split('.')[1]) || 1,
                    ipAddress: parts[2],
                    subnetMask: parts[3],
                  });
                }
        ...
        if (parts[1] === 'mode' && parts[2] === 'access') {
          ...
          updateNode(device.id, { interfaces });
          newHistory.push('');
        } else if (parts[1] === 'mode' && parts[2] === 'trunk') {
          ...
          updateNode(device.id, { interfaces });
          newHistory.push('');
        } else if (parts[1] === 'trunk' && parts[2] === 'allowed' && parts[3] === 'vlan') {
          // Set allowed VLANs on trunk: switchport trunk allowed vlan 10,20,30
          if (parts.length >= 5) {
            const vlanList = parts[4].split(',').map(v => parseInt(v.trim())).filter(v => !isNaN(v));
            const interfaces = device.data.interfaces.map(iface =>
              iface.name === currentInterface && iface.trunkMode
            ...
            newHistory.push('% Usage: switchport trunk allowed vlan <vlan-list>');
          }
        } else if (parts[1] === 'trunk' && parts[2] === 'native' && parts[3] === 'vlan') {
          // Set native VLAN: switchport trunk native vlan 10
          const vlanId = parseInt(parts[4]);
          if (!isNaN(vlanId)) {
            const interfaces = device.data.interfaces.map(iface =>
              iface.name === currentInterface && iface.trunkMode
          ...
          if (!isNaN(vlanId)) {
            ...
            updateNode(device.id, { vlans: updatedVlans, interfaces });
            newHistory.push('');
          }
        } else {
          newHistory.push('% Invalid switchport command');
        }
      } else if (command === 'encapsulation' && device.data.type === 'router') {
        // For sub-interfaces: encapsulation dot1Q <vlan-id>
        if (parts[1] === 'dot1q' || parts[1] === 'dot1Q') {
          const vlanId = parseInt(parts[2]);
          if (!isNaN(vlanId) && vlanId >= 1 && vlanId <= 4094) {
            // currentInterface should be like "Gig0/0.10"
            const [parentName] = currentInterface!.split('.');
            const interfaces = device.data.interfaces.map(iface => {
              if (iface.name === parentName) {
                // Find or create sub-interface
                const subInterfaces = iface.subInterfaces || [];
                const existingSubIdx = subInterfaces.findIndex(si => si.name === currentInterface);
                
                if (existingSubIdx >= 0) {
                  // Update existing sub-interface
          ...
          newHistory.push('% Invalid encapsulation type. Use: encapsulation dot1Q <vlan-id>');
        }
      } else if (trimmed === '?' || trimmed === 'help') {
        const baseCommands = [
        ...
        const routerCommands = device.data.type === 'router' && currentInterface?.includes('.') ? [
